...
The repository now includes an RL orientation, curated readings, and a guided **CartPole onboarding assignment** with starter code, section checks, and a report template.

## Start here

1. **[Complete the orientation](ORIENTATION.md)** — allow roughly 60–90 minutes to learn the shared RL vocabulary. No installation, coding, or prior RL experience is required.
2. **[Work through CartPole onboarding](ONBOARDING.md)** — build a MuJoCo model, connect it to a Gymnasium task, train a PPO policy, and evaluate it against random actions. You need basic Python functions, classes, and imports; the guide introduces the Git workflow. Budget about 8–12 focused hours over two weeks, with a third week for setup or debugging. This estimate still needs a beginner pilot.
3. **Complete your handoff** — fill in [REPORT.md](REPORT.md), review and merge your assignment PR within your own fork, and share the completed repository with the RL lead. Follow the [handoff instructions](ONBOARDING.md#6-report-and-github-handoff), then move to a separate project repository when you receive a team assignment.
...
See the [setup guide](ONBOARDING.md#1-setup-and-git-workflow) for installation and platform requirements, including WSL2 for Windows and the pending Apple Silicon runtime pilot.

## Repository guide

| Location | What you will find |
| --- | --- |
| [ORIENTATION.md](ORIENTATION.md) | First readings and vocabulary checklist |
| [ONBOARDING.md](ONBOARDING.md) | Assignment steps, setup, checks, and submission instructions |
| [REPORT.md](REPORT.md) | Template for explaining your task, experiment, and results |
...
Keep guided learning material, curated references, and small onboarding exercises here. CartPole introduces the modeling, training, and evaluation workflow that members will later use in humanoid-soccer work.

Production robot skills and long-running experiments belong in separate project repositories, which this hub can link to as they are created. Member solutions stay in member forks. Keep trained policies, datasets, logs, and videos out of Git; the onboarding guide explains which compact results to commit.
